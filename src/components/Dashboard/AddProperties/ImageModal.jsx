    ...
    <>
      ...
      >
        <div className={`${style.main_img_container} me-3`}>
          <img src={img.url} className={`${style.main_img}`} alt="img" />
        </div>
        <div>
          <p>
            <span className="text-primary"> Name </span> - {img?.name}{" "}
          </p>
          <p>
            <span className="text-primary"> Format </span> - {img?.format}{" "}
          </p>
          <p>
          ...
          <p>
            <span className="text-primary"> Type </span> - {img?.type}{" "}
          </p>
        </div>
...
export const MoreImages = ({ open, Property, setOpen, ToggleModal }) => {
  ...
  const updateImage = async () => {
    try {
      const response = await axios.put(
        `https://celahl.herokuapp.com/api//property/update-images/${Property?._id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${token} `,
      ...
      >
        <div className="">
          ...
          <div className="d-flex justify-content-between my-5">
            <button onClick={ToggleModal} className="btn btn-danger mx-5">
              {" "}
              close{" "}
            </button>
            <button onClick={updateImage} className="btn btn-primary">
              {" "}
              update{" "}
            </button>
          </div>
